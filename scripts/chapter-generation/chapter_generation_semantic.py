...
def _model_name() -> str:
    ...
    return (os.environ.get("VOD_CHAPTER_EMBED_MODEL") or "sentence-transformers/all-MiniLM-L6-v2").strip()


def _model_device() -> str:
    return (os.environ.get("VOD_CHAPTER_DEVICE") or "cuda").strip()
...
def _model():
    ...
    try:
        ...
        if device != "cpu":
            print(f"[chapter-generation] semantic model init failed on {device}; retrying on cpu: {exc}", file=sys.stderr, flush=True)
