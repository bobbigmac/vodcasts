...
def llm_chaptering_enabled() -> bool:
    raw = (os.environ.get("VOD_CHAPTER_LLM") or "1").strip().lower()
    return raw not in {"", "0", "false", "no", "off"}


def _llm_provider() -> str:
    raw = (os.environ.get("VOD_CHAPTER_LLM_PROVIDER") or "").strip().lower()
    if raw in {"openai", "local"}:
        return raw
    model = (os.environ.get("VOD_CHAPTER_LLM_MODEL") or "").strip()
...
def _openai_model_name() -> str:
    raw = (os.environ.get("VOD_CHAPTER_OPENAI_MODEL") or os.environ.get("OPENAI_MODEL") or "").strip()
    if raw:
        return raw
    model = (os.environ.get("VOD_CHAPTER_LLM_MODEL") or "").strip()
    if model.lower().startswith("openai:"):
...
def _model_name() -> str:
    ...
    return (os.environ.get("VOD_CHAPTER_LLM_MODEL") or _DEFAULT_MODEL).strip()


def _device() -> str:
    if _llm_provider() == "openai":
        return "api"
    forced = (os.environ.get("VOD_CHAPTER_LLM_DEVICE") or "").strip()
    if forced:
        return forced
...
def _max_input_chars() -> int:
    try:
        return max(1000, int(os.environ.get("VOD_CHAPTER_LLM_MAX_INPUT_CHARS") or "2400"))
    except Exception:
        return 2400


def _remote_url() -> str:
    return (os.environ.get("VOD_CHAPTER_LLM_URL") or "").strip().rstrip("/")


def _server_mode() -> bool:
    raw = (os.environ.get("VOD_CHAPTER_LLM_SERVER") or "").strip().lower()
...
def _http_timeout_sec() -> float:
    try:
        return max(5.0, float(os.environ.get("VOD_CHAPTER_LLM_HTTP_TIMEOUT_SEC") or "180"))
    except Exception:
        return 180.0
...
def _review_boundary_local(*, before_text: str, after_text: str, title_hint: str = "") -> BoundaryDecision | None:
    ...
    try:
        ...
        print(f"[chapter-generation] LLM boundary review unavailable; falling back: {exc}", file=sys.stderr, flush=True)
        return None
...
def _refine_chapter_metadata_local(
    *,
    ...
    try:
        ...
        print(f"[chapter-generation] LLM chapter refinement unavailable; falling back: {exc}", file=sys.stderr, flush=True)
        return None
    if not isinstance(raw, dict):
        return None
...
def review_boundary(*, before_text: str, after_text: str, title_hint: str = "") -> BoundaryDecision | None:
    ...
    if _use_remote():
        try:
            ...
            print(f"[chapter-generation] LLM remote boundary review failed; falling back local: {exc}", file=sys.stderr, flush=True)
    ...
    *,
    ...
    if _use_remote():
        try:
            ...
            print(f"[chapter-generation] LLM remote chapter refinement failed; falling back local: {exc}", file=sys.stderr, flush=True)
...
def _plan_query_local(*, question: str) -> QueryPlan | None:
    ...
    try:
        ...
        print(f"[chapter-generation] LLM query planning unavailable: {exc}", file=sys.stderr, flush=True)
        return None
    if not isinstance(raw, dict):
        return None
...
def _review_answer_candidate_local(
    *,
    ...
    try:
        ...
        print(f"[chapter-generation] LLM answer review unavailable: {exc}", file=sys.stderr, flush=True)
        return None
    if not isinstance(raw, dict):
        return None
    return AnswerReview(
...
def plan_query(*, question: str) -> QueryPlan | None:
    ...
    if _use_remote():
        try:
            ...
            print(f"[chapter-generation] LLM remote query planning failed; falling back local: {exc}", file=sys.stderr, flush=True)
    ...
    *,
    ...
    if _use_remote():
        try:
            ...
            print(f"[chapter-generation] LLM remote answer review failed; falling back local: {exc}", file=sys.stderr, flush=True)
...
def _summarize_answer_candidate_local(
    *,
    ...
    try:
        ...
        print(f"[chapter-generation] LLM answer summary unavailable: {exc}", file=sys.stderr, flush=True)
        return None
    if not isinstance(raw, dict):
        return None
    ...
    *,
    ...
    if _use_remote():
        try:
            ...
            print(f"[chapter-generation] LLM remote answer summary failed; falling back local: {exc}", file=sys.stderr, flush=True)
