...
def analyze_transcripts(
    *,
    ...
    log(f"[chapter-generation] analyzing transcripts: {transcripts_root} (files={len(files)})")
    log(f"[chapter-generation] db: {db_path}")
    log(f"[chapter-generation] mode: incremental={bool(incremental)} force={bool(force)}")
    ...
    try:
        for idx, p in enumerate(files, 1):
            ...
            if incremental and not force and _file_row_is_current(p, row):
                skipped += 1
                log(f"[chapter-generation] [{idx}/{total_n}] skip  {rel}")
                continue
            ...
            touched += 1
            log(f"[chapter-generation] [{idx}/{total_n}] update {rel} cues={len(cues)} segs={len(segs)}")
            if not quiet and (touched + skipped) % 25 == 0:
                elapsed = max(0.001, time.time() - started)
                rate = (touched + skipped) / elapsed
                log(f"[chapter-generation] progress: updated={touched} skipped={skipped} rate={rate:.1f} files/s")
        ...
        elapsed = max(0.001, time.time() - started)
        log(f"[chapter-generation] done: updated={touched} skipped={skipped} elapsed={elapsed:.1f}s")
    except KeyboardInterrupt:
        elapsed = max(0.001, time.time() - started)
        log(f"\n[chapter-generation] interrupted: updated={touched} skipped={skipped} elapsed={elapsed:.1f}s")
        raise
    finally:
        try:
            con.close()
...
def rebuild_search_index(*, db_path: Path, quiet: bool = False) -> None:
    ...
    try:
        ...
        elapsed = max(0.001, time.time() - started)
        log(f"[chapter-generation] indexed {total_segments} segments into FTS in {elapsed:.1f}s")
    finally:
        try:
            con.close()
        except Exception:
...
def write_chapters_from_analysis(
    *,
    ...
    log(f"[chapter-generation] writing chapters from analyzed segments (files={len(rows)})")
    wrote = 0
    skipped = 0
    stale = 0
    try:
        for idx, row in enumerate(rows, 1):
            ...
            if target and not force and target.exists() and not _chapters_needs_update(target, mode=str(mode or "semantic")):
                skipped += 1
                log(f"[chapter-generation] [{idx}/{len(rows)}] skip  {rel}")
                continue
            ...
            wrote += 1
            log(f"[chapter-generation] [{idx}/{len(rows)}] write {rel} chapters={len(chapters.get('chapters') or [])}")
    finally:
        try:
            con.close()
        except Exception:
    ...
    if not quiet:
        print(f"[chapter-generation] done: wrote={wrote} skipped={skipped} stale={stale}", flush=True)
...
def answer_question(
    *,
    ...
    from chapter_generation_llm import plan_query, summarize_answer_candidate
...
def _chapter_title(kind: str, text: str, *, conf: float | None = None, title_mode: str = "semantic") -> str:
    ...
    from chapter_generation_semantic import keyphrases_for_title, representative_sentence  # type: ignore
...
def chapters_from_segments(*, feed: str, episode_slug: str, segments: list[Segment], mode: str = "semantic") -> dict[str, Any]:
    ...
    if title_mode in {"semantic", "embed", "embeddings", "hybrid"} and main_start > 0.0 and content_end - main_start >= 900.0:
        from chapter_generation_semantic import TextSpan, pick_chapter_times  # type: ignore
        ...
        for t in times:
            ...
            if txt:
                ...
                if llm_enabled:
                    try:
                        from chapter_generation_llm import review_boundary  # type: ignore
                        ...
                        print(f"[chapter-generation] LLM boundary review failed at {t:.1f}s: {exc}", flush=True)
    ...
    if llm_enabled and chapters:
        try:
            from chapter_generation_llm import refine_chapter_metadata  # type: ignore
            ...
            print(f"[chapter-generation] LLM chapter refinement failed; falling back to extractive tags: {exc}", flush=True)
            for idx, ch in enumerate(chapters):
                if ch.get("tags"):
                    continue
                start_t = float(ch.get("startTime") or 0.0)
    ...
    return {
        "version": 1,
        "generator": {"name": "vodcasts-chapter-generation", "version": 1, "mode": str(mode or "semantic")},
...
def _chapters_needs_update(path: Path, *, mode: str) -> bool:
    try:
        ...
        if not isinstance(gen, dict):
            return True
        if str(gen.get("name") or "") != "vodcasts-chapter-generation":
            return True
        if int(gen.get("version") or 0) != 1:
            return True
